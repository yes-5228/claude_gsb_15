...
面向城市公厕管养单位的巡查记录与整改闭环管理系统，覆盖 **公厕台账 → 保洁巡查 → 问题上报 → 整改跟踪 → 应急处置** 五条业务主线。后端为 FastAPI + SQLAlchemy，前端为 React + Vite，前后端均按模块拆分，可单独开发、单独部署。

## 功能模块

| 模块 | 页面/入口 | 主要能力 |
| --- | --- | --- |
| 总览看板 | `/` | 核心指标卡、巡查与问题趋势、整改状态/分类/严重程度分布、应急事件类型分布与最新事件、区域运行情况、重点关注公厕、最新问题与巡查 |
...
| 应急处置 | `/emergencies`、`/emergencies/:id` | 停水停电/设施爆裂/污损外溢等事件登记、按类型约定响应时限、响应耗时计算与超时预警、处置流转、恢复情况与影响补充、处置轨迹时间线 |
...
| GET | `/emergencies` | 应急事件查询（status/event_type/district/overdue/open_only/日期区间/关键字） |
| POST | `/emergencies` | 登记应急事件，自动生成编号 `YJ-YYYYMMDD-001`，响应时限按事件类型带出（可覆盖） |
| GET/PATCH/DELETE | `/emergencies/{id}` | 详情（含响应耗时、处置轨迹）/ 更新 / 删除 |
| GET | `/emergencies/{id}/transitions` | 当前状态可执行的处置动作 |
| POST | `/emergencies/{id}/transitions` | 推进处置状态（响应/恢复/关闭，恢复时必填恢复情况） |
| POST | `/emergencies/{id}/records` | 追加处置记录（不改变状态） |
| GET | `/stats/overview` | 核心指标 |
| GET | `/stats/dashboard` | 看板聚合数据（趋势、分布、区域、排行、最新记录） |
| GET | `/meta/dictionaries` | 枚举字典（状态、分类、程度、检查项、流转规则、应急类型与响应时限约定） |
...
- **应急事件编号**：`YJ-` + 发现日期 + 当日三位流水号。
- **响应时限约定**：按事件类型预设响应时限（停水停电 30 分钟、设施爆裂 20 分钟、污损外溢 40 分钟、其他 60 分钟），登记时自动带出、可单独调整；修改事件类型且未显式指定时限时会按新类型重新约定。响应耗时 = 响应时间 − 发现时间，实际响应晚于时限（或逾时限仍未响应）即标记「响应超时」，列表可筛选、看板统计超时数量。
- **应急处置闭环**：`待响应 → 处置中 → 已恢复 → 已关闭`；`待响应/处置中` 可直接关闭。响应时记录响应时间并计算响应耗时；流转到「已恢复」时必须补充恢复情况，可同时登记造成的影响，事后仍可通过编辑补充；每次流转写入一条处置流水，详情页以时间线呈现。
- **删除保护**：删除公厕时若已存在巡查、问题或应急事件记录，接口返回 409 并提示数量，需要显式 `force=true` 才会级联删除；前端会二次确认。

## 演示数据

`SEED_ON_STARTUP=true`（默认）且数据库为空时，会自动写入：10 座公厕（4 个区域、三类等级、含维修/停用状态）、近 14 天约 90 条巡查记录、13 条不同整改阶段的问题及其完整整改轨迹、6 起覆盖各事件类型与处置阶段的应急事件（含响应超时示例）。数据由固定随机种子生成，结果可复现；如需重置，删除 `backend/data/app.db`（或 `docker compose down -v`）后重启即可。
