...
def delete_restroom(db: Session, restroom_id: int, *, force: bool = False) -> None:
    ...
    emergency_count = db.scalar(
        select(func.count()).select_from(EmergencyEvent).where(EmergencyEvent.restroom_id == restroom_id)
    ) or 0
    if (inspection_count or issue_count or emergency_count) and not force:
        raise ConflictError(
            f"该公厕已有 {inspection_count} 条巡查记录、{issue_count} 条问题记录、"
            f"{emergency_count} 条应急事件，确需删除请使用 force=true"
        )
    db.delete(restroom)
    db.commit()
...
def get_restroom_detail(db: Session, restroom_id: int) -> RestroomDetail:
    ...
    open_emergency_count = db.scalar(
        ...
        .where(
            EmergencyEvent.restroom_id == restroom_id,
            EmergencyEvent.status.in_(OPEN_EMERGENCY_STATUSES),
        )
    ) or 0
